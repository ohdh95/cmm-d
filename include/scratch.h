...
template <typename T> class SSDQueryScratch : public AbstractScratch<T>
{
  public:
    ...
    SSDQueryScratch(size_t aligned_dim, size_t visited_reserve, size_t max_node_len = 0);
    ~SSDQueryScratch();
...
template <typename T> class SSDThreadData
{
  public:
    ...
    SSDThreadData(size_t aligned_dim, size_t visited_reserve, size_t max_node_len = 0);
    void clear();
};
